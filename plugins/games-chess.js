            ...
            switch (args[0].toLowerCase()) {
              case "c":
                case "challenge":
                 ...
                 m { contextInfo: {mentionedJid: [m.sender,who] }})
                 break
                  case "a":
                    case "accept":
                      ...
                      if ( challenge.who !== m.sender) return m.reply('Tidak ada seseorang yang mengajakmu bermain catur!')
                       ongoing.add(m.chat)
                      const game = new Game(new EventEmitter(), m.chat)
                     await conn.reply(m.chat,
                              `*Game Catur Dimulai!*\n\n⬜ *Putih:* @${challenge.challenger.split('@')[0]}\n⬛ *Hitam:* @${
                                    challenge.who.split('@')[0]
                    }`, m, { contextInfo: {mentionedJid: [m.sender, challenge.challenger]}})
                    game.start(print, challenge.challenger, challenge.who, async () => {
                        ...
                        if (!sent) {
                            try {
                               var buff = await cig
                                    .generateBuffer()
                            await conn.sendFile(m.chat, buff, 'c.jpg', null, m)
                                sent = true
                            } catch (err) {
                                m.reply(err)
                            }
                ...
                case "m":
                    ...
                    if (args[1] == 'castle') {
                                        ...
                                        return g.eventEmitter.emit(m.chat, move, print, m.sender, () => async () => {
                                            ...
                                            if (!sent) {
                                                try {
                                                    await cig.generateBuffer().then(async (data) => await conn.sendFile(m.chat, data, 'c.jpg', null, m))
                                                    sent = true
                                                } catch (err) {
                                                    m.reply(err)
                                                }
