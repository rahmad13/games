...
*${usedPrefix + command} start* = start game`)
  ...
  if (args[0] == "money"){
    ...
    if(m.sender == conn.war[m.chat][0].user){
      ...
      return conn.reply(m.chat,`*Hanya @${conn.war[m.chat][0].user.split('@')[0]} sebagai pembuat room yang bisa mengganti modal awal perang*`,m, {mentions: [conn.war[m.chat][0].user]})
    }
  }
