...
let monsters = [
  ...
  if (new Date - global.db.data.users[m.sender].lasthunt > 120000) {
    ...
    if (player.healt < 0) {
      let msg = `*@${pengirim}* Anda Mati Di Bunuh🗡️ Oleh ${monsterName}👹`
      if (player.level > 0) {
        player.level -= 1
    ...
    m.reply(pesan)
  } else throw `Tunggu *00:0${cd1}:${cd2}* Untuk Berburu🗡️ Lagi!`
}
handler.help = ['hunt', 'ht']
handler.tags = ['rpg']
handler.command = /^(hunt|ht|hun)$/i
