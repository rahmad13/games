...
async function downloadMulti(url) {
  ...
  let result = await page.evaluate(() => {
    return {
      "title": document.querySelector("div.title").innerHTML,
      "url": document.querySelector("a.link.link-download").getAttribute("href")
    }
  })
  await page.close()
  return result
...
async function stalkCOC(tag) {
  let browser = await puppeteer.launch({ args: ["--no-sandbox"] })
  let page = await browser.newPage()
  await page.goto(`https://pixelcrux.com/Clash_of_Clans/Profile/App?tag=${tag}`)
  await page.waitForTimeout(3000)
  let _html = await page.content()
  let _$ = cheerio.load(_html)

  let html = _$("div.module-body").html()
  let { document } = (new JSDOM(html)).window

  return {
    id: document.querySelector("div.label-aside > span").innerHTML,
    nama: document.querySelector("h3.xl-header").innerHTML,
    update: document.querySelectorAll("div > span")[2].innerHTML.replace(/( \<\!\-\- [A-z]{2,5}\: [A-z]{2,256} \-\-\>)/g, ""),
    level: document.querySelectorAll("ul.profile-list > li > div")[1].innerHTML.split("<")[0],
    exp: document.querySelectorAll("ul.profile-list > li > div")[1].innerHTML.split(">")[2].split(" ")[0],
    thropies: document.querySelectorAll("ul.profile-list > li > div")[3].innerHTML.replace(/(\<br\>\<[A-z ="-]{2,256})/g, "").replace(">", " | ").split("<")[0]
  }
}
...
const byMe = { mediafire, primbon, emojipedia, downloadMulti, stalkCOC, stalkGithub, google }

export default { facebook, quotes, igdl, igstory, tiktok, twitter, joox, pin, nhentai,  tikolu, anime, byMe }
